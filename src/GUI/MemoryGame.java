...
public class MemoryGame extends JFrame implements ActionListener {
  ...
  private ArrayContainer<String> container =
          new ArrayContainer<>(ROWS_IN_GAME * COLS_IN_GAME);
  ...
  /**
   * Initialize the game board: size it, store the symbols, create the buttons,
   * give each button a symbol to display and place the buttons on the panel.
     * @throws exceptions.EmptyContainerException
   */
  public MemoryGame() throws EmptyContainerException {
   ...
   */
  private void init() throws EmptyContainerException {
    String output = "";
    ...
    try {
      for (int i = 0; i < ROWS_IN_GAME * COLS_IN_GAME; i++) {
        btnA = new TileButton((String) (container.removeRandom()));  // create button
      ...
      JOptionPane.showMessageDialog(null,e.getMessage(), "Bag is empty"
  ...
  /**
   * This methods has no purpose in the memory game application.
   * This method tests size, toString, isEmpty, remove and removeRandom
   * methods from the ArrayContainer class
   */
  private void testMethods1() throws EmptyContainerException{
    String output = "";
   try {
   ...
   while (!container.isEmpty()) {
      String element = container.removeRandom();
      output += element + ", ";
      container.remove(element);
    }
   } catch(EmptyContainerException e) {
       JOptionPane.showMessageDialog(null, "Empty container!");
   }
